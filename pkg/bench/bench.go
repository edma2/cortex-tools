...
func NewBenchRunner(cfg Config, logger log.Logger, reg prometheus.Registerer) (*Runner, error) {
	...
	workload := newWriteWorkload(workloadDesc, cfg.InstanceName, prometheus.DefaultRegisterer)
	...
	if cfg.Query.Enabled {
		queryWorkload, err := newQueryWorkload(cfg.ID, workloadDesc, cfg.InstanceName)
		if err != nil {
